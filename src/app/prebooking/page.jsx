...
import { User, Mail, Phone, MapPin, Globe, Smartphone, Lock, Calendar, MessageSquare, CheckCircle } from "lucide-react";
...
export default function PreBookingForm() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-200 py-12 px-4 sm:px-6 lg:px-8 flex items-center justify-center">
      ...
      <div
        ref={formRef}
        className="w-full max-w-3xl bg-white rounded-2xl shadow-lg p-6 sm:p-8"
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
              <Lock size={18} /> Would you like to join beta testing?
            </label>
            <div className="flex gap-4">
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="radio"
                  value="Yes"
                  {...register("betaTester", { required: "Please select an option" })}
            ...
            {errors.betaTester && <p className="text-red-500 text-sm mt-1">{errors.betaTester.message}</p>}
          </div>
